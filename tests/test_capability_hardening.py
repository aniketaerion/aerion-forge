...
def test_exact_approved_catalogue_and_real_contract_paths() -> None:
    ...
    assert sum(item.lifecycle is CapabilityLifecycle.PLANNED for item in catalogue) == 24
...
def test_query_contract_is_sorted_read_only_and_complete() -> None:
    ...
    assert (
        len(query.list_available_capabilities()) == 7
        and len(query.list_planned_capabilities()) == 24
    )
