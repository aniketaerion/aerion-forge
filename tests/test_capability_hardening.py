...
def test_exact_approved_catalogue_and_real_contract_paths() -> None:
    ...
    assert sum(item.lifecycle is CapabilityLifecycle.PLANNED for item in catalogue) == 16
    ...
    subsystem = {
        ...
        "safe-change-planning": "safe_change_planning",
    }
...
def test_query_contract_is_sorted_read_only_and_complete() -> None:
    ...
    assert (
        len(query.list_available_capabilities()) == 15
        and len(query.list_planned_capabilities()) == 16
    )
    ...
    assert query.is_available("task-management")
    assert query.get_missing_requirements("task-management") == ()
